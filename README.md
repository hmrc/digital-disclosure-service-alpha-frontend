...
The hub lists three journeys (Option 1 Unclaimed enrolment, Option 2 client-registers-first, digitally excluded) with pros/cons from the v2 approaches doc. A stand-in banner explains that `HMRC-MTD-IT` / `mtd-it-auth` replace the production DDS regime and `dds-auth` rule.
